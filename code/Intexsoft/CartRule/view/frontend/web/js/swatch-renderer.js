    ...
    return function (SwatchRenderer) {
        ...
        $.widget('mage.SwatchRenderer', $['mage']['SwatchRenderer'], {
            ...
            _getPrices: function (newPrices, displayPrices) {
                var $widget = this;
                let reprice = displayPrices["baseOldPrice"].amount;
                if (_.isEmpty(newPrices)) {
                    newPrices = $widget._getNewPrices();
                }
                _.each(displayPrices, function (price, code) {
                    if (newPrices[code]) {
                        if (displayPrices[code].amount === reprice && (cartTotal.textContent !== null && cartTotal.textContent > 100)) {
                            displayPrices[code].amount = newPrices[code].amount - displayPrices[code].amount - (displayPrices[code].amount * 10) / 100;
